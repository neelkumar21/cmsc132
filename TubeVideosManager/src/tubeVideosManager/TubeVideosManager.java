...
public class TubeVideosManager implements TubeVideosManagerInt {
	...
	public ArrayList<Video> getAllVideosInDB() {
		ArrayList<Video> database = new ArrayList<Video>();
		for (int i = 0; i < videoDatabase.size(); i++) {
			database.add(videoDatabase.get(i));
		}
		return database;
	}
	...
	public boolean addComments(String title, String comments) {
		if (title == null || title.isBlank() || comments == null || comments.isBlank()) {
			...
			findVideo(title).addComments(comments);
			return true;
		}
	}
	...
	public boolean addVideoToPlaylist(String title, String playlistName) {
		if (findVideo(title) == null || findPlayListInternal(playlistName) == null) {
			return false;
		}
	...
	public Playlist searchForVideos(String playlistName, String title, int maximumDurationInMinutes, Genre genre) {
		Playlist playlist = new Playlist(playlistName);
		for (int i = 0; i < videoDatabase.size(); i++) {
			if (videoDatabase.get(i).getTitle().equals(title) || title == null) {
				if (videoDatabase.get(i).getDurationInMinutes() < maximumDurationInMinutes
						|| maximumDurationInMinutes < 0) {
					if (videoDatabase.get(i).getGenre().equals(genre) || genre == null) {
